...
type View = 'inicio' | 'tienda' | 'citas' | 'pacientes' | 'operaciones' | 'administracion'
    ...
    <main className="min-h-screen bg-[#f7f9f7] text-[#173b3b]">
      <header className="sticky top-0 z-40 border-b border-[#dce7e2] bg-[#f7f9f7]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1440px] items-center justify-between gap-6 px-6 py-4 lg:px-10">
          ...
          <nav className="hidden items-center gap-8 text-sm font-medium text-[#66817a] lg:flex">
            {([['inicio', 'Inicio'], ['tienda', 'Pet shop'], ['citas', 'Citas'], ['pacientes', 'Mis mascotas'], ['operaciones', 'Panel operativo']] as [View, string][]).map(([id, label]) => <button key={id} onClick={() => setView(id)} className={`transition-colors hover:text-[#0d5c5b] ${view === id ? 'font-bold text-[#0d5c5b]' : ''}`}>{label}</button>)}
          </nav>
      ...
      {view === 'tienda' && <Shop products={filteredProducts} search={search} setSearch={setSearch} category={category} setCategory={setCategory} addToCart={addToCart} />}
      ...
      {view === 'operaciones' && <Operations role={role} setRole={setRole} showToast={showToast} setView={setView} />}
...
function Metric({ icon: Icon, label, value, note }: { icon: React.ElementType; label: string; value: string; note: string }) { return <div className="rounded-2xl bg-white p-5 ring-1 ring-[#e1ebe6]"><div className="flex items-center justify-between"><span className="text-xs font-semibold text-[#829990]">{label}</span><Icon className="size-4 text-[#d37c52]" /></div><p className="mt-3 font-serif text-2xl font-bold text-[#173b3b]">{value}</p><p className="mt-1 text-xs text-[#7b9b8b]">{note}</p></div> }
